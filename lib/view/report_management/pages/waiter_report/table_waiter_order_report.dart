...
class TableWaiterOrderReport extends StatefulWidget {
  final String type;
  final List transaction;
  const TableWaiterOrderReport(
      {super.key, required this.type, required this.transaction});

  @override
  State<TableWaiterOrderReport> createState() => _TableWaiterOrderReportState();
}

class _TableWaiterOrderReportState extends State<TableWaiterOrderReport> {
  @override
  ...
  List<DataRow> _buildRows() {
    ...
    List successfulTransactions = widget.transaction
        .where((transaction) => transaction['transaction_status'] == 'sukses')
        .toList();
    int totalQty = 0;
    ...
    for (var entry in sortedGroups) {
      ...
      rows.add(DataRow(cells: [
        DataCell(Text((rows.length + 1).toString())),
